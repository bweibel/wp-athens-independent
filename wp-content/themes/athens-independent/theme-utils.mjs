...
async function escapePatterns(patternsAndThemes, options = {}) {
  ...
  function escapeImagePath(src) {
    ...
    // This regex handles URLs like https://athensindependent.local/wp-content/themes/athens-independent/patterns/images/avatar-7.webp
...
async function watchPatterns(options = {}) {
  ...
  const textDomain = options.textDomain || 'athens-independent';
...
Examples:
  node theme-utils.mjs escape-patterns --text-domain=athens-independent
  node theme-utils.mjs watch --text-domain=athens-independent
  node theme-utils.mjs escape-patterns --debug --text-domain=athens-independent
  `);
}
